...
st.write(f"**Ley CC Zn (%):** {ley_cc_zn:.4f}")
st.write(f"**Recuperación Zn (%):** {rec_zn:.4f}")
st.write(f"**Recuperación Zn duplicado (%):** {rec_zn_dup:.4f}")
st.write(f"**TMS Concentrado Zinc:** {tms_zn:,.2f}")
...
st.write(f"**Ley CC Pb (%):** {ley_cc_pb:.4f}")
st.write(f"**Recuperación Pb (%):** {rec_pb:.4f}")
st.write(f"**Recuperación Ag en Pb (%):** {rec_ag_pb:.4f}")
st.write(f"**Ley CC Ag en Pb (oz/ton):** {ley_cc_ag_pb:.4f}")
st.write(f"**TMS Concentrado Plomo:** {tms_pb:,.2f}")
...
st.write(f"**Ley CC Cu (%):** {ley_cc_cu:.4f}")
st.write(f"**Recuperación Cu (%):** {rec_cu:.4f}")
st.write(f"**Recuperación Ag en Cu (%):** {rec_ag_cu:.4f}")
st.write(f"**Ley CC Ag en Cu (oz/ton):** {ley_cc_ag_cu:.4f}")
st.write(f"**TMS Concentrado Cobre:** {tms_cu:,.2f}")
...
st.write(f"**Total Concentrado (TMS):** {total_concentrado:,.2f}")
st.write(f"**Finos de Zinc (TMS):** {finos_zn:,.2f}")
st.write(f"**Finos de Plomo (TMS):** {finos_pb:,.2f}")
st.write(f"**Finos de Cobre (TMS):** {finos_cu:,.2f}")
st.write(f"**Finos de Plata (Oz):** {finos_ag:,.2f}")
...
st.write(f"**Costo Variable Mina (US$/TM):** {costo_var_mina_tm:,.2f}")
st.write(f"**Costo Variable Planta - G&A (US$):** {costo_var_planta_ga:,.2f}")
st.write(f"**Producción (TMD):** {produccion_tmd:,.2f}")
st.write(f"**Precio $ (T/Zn eq):** {precio_zn_eq:,.2f}")
st.write(f"**Días laborados:** {dias}")
